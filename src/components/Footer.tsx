...
const SocialLinks = ({ social }: { social?: SocialSettings | null }) => {
  ...
  return (
    <>
      ...
      {social.rss && (
        <a href={social.rss} target="_blank" rel="external noopener noreferrer" className="p-2 md:text-slate-400 text-slate-500 hover:text-orange-500 md:hover:bg-orange-50 md:dark:hover:bg-orange-500/10 rounded-lg transition-all" title="RSS">
          <span className="sr-only">RSS</span>
          <Rss className="w-6 h-6 md:w-5 md:h-5 sm:w-4 sm:h-4" />
        </a>
      )}
    </>
  );
